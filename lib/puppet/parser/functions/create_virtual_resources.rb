Puppet::Parser::Functions::newfunction(:create_virtual_resources, :doc => '
...
') do |args|
  ...
  if type_name == 'class'
    raise ArgumentError, "create_virtual_resources does not support classes."
  else
  ...
  args[1].each do |title, params|
    ...
    when :define
      #raise ArgumentError, "could not create virtual define resources"
      p_resource = Puppet::Parser::Resource.new(type_name, title, :scope => self, :source => resource)
      p_resource.virtual = true
